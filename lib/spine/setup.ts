...
export const SETUP_ITEMS: SetupItem[] = [
  {
    ...
    blocks:
      'Searching your name puts mikecalo.co above calo.company. The domain lapses on 27 September rather than being redirected, so the old result goes away on its own and hands nothing across. Everything after that is making calo.company rank for Mike Calo on its own strength.\n\nThe founder page is already good: the title reads Mike Calo, Product Marketing and GTM Leader, the name appears ten times, and LinkedIn is linked. Two real gaps remain, and they are both in the head rather than the writing.',
    steps: [
      'Nothing to renew. Auto renew is already off on both the domain and the plan, so doing nothing is the whole action.',
      'While you still own it, verify mikecalo.co in Search Console and use the Removals tool. That takes the old result out of Google in about a day instead of the weeks it takes Google to notice a dead domain. It only works while the domain is still yours.',
      'Unpublish the Wix site now. In the Wix editor the menu is Site, then Unpublish. If you cannot find it in Studio, disconnecting mikecalo.co from the site in Settings, Domains does the same job for anybody searching.',
      'Verify calo.company in Search Console. It keeps no history from before the day you verify, so the sooner it is on the better the before looks.',
      'Add Person structured data to calo.company/mike-calo. The page has two JSON-LD blocks and both are about the company: one is an FAQ about CALO&CO. Nothing on the page tells Google it is about a person, which is the single biggest gap for a name query.',
      'In that Person block, use sameAs pointing at linkedin.com/in/mikecalo. The page already links LinkedIn for a reader; sameAs is what says to Google that the profile and the page are the same person. LinkedIn outranks both sites for your name, so it is the only authority you inherit.',
      'Put the full name in the H1. It currently reads "Hey, I\u2019m Mike, and I\u2019m a leader", which is a good line and does not contain the thing being searched for. The title tag has it; the heading should too.',
      'Add the name to the meta description. It currently opens with thirteen years turning products into categories and never says who.',
      'Expect months rather than weeks. Without a redirect you are building name authority from nothing, and the Search Console position graph for "mike calo" is the only honest measure of it.',
    ],
  },
  {
    key: 'search_console',
