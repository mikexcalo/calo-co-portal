...
export default function AllInvoicesPage() {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, { bg: string; color: string; label: string }> = {
      paid: { bg: `rgba(0,200,83,0.1)`, color: t.status.success, label: 'Paid' },
      overdue: { bg: `rgba(255,61,61,0.1)`, color: t.status.danger, label: 'Overdue' },
      unpaid: { bg: `rgba(255,171,0,0.1)`, color: t.status.warning, label: 'Unpaid' },
    };
    const s = map[status] || map.unpaid;
    return <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: s.bg, color: s.color }}>{s.label}</span>;
  };
