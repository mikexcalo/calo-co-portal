'use client';

import { useTheme } from '@/lib/theme';

type NautilusSpinnerProps = {
  spinning?: boolean;
  size?: number;
};

export default function NautilusSpinner({ spinning = false, size = 28 }: NautilusSpinnerProps) {
  const { t } = useTheme();
  return (
    <div style={spinning ? { animation: 'nautilus-spin 2.5s linear infinite', display: 'inline-flex' } : { display: 'inline-flex' }}>
      <svg
        width={size}
        height={size}
        viewBox="0 0 24 24"
        fill="none"
        stroke={t.text.tertiary}
        strokeWidth="1.2"
        strokeLinecap="round"
      >
        <circle cx="12" cy="12" r="8.5" />
        <circle cx="12" cy="12" r="3" />
        <line x1="12" y1="3.5" x2="12" y2="9" />
        <line x1="12" y1="15" x2="12" y2="20.5" />
        <line x1="3.5" y1="12" x2="9" y2="12" />
        <line x1="15" y1="12" x2="20.5" y2="12" />
        <line x1="5.9" y1="5.9" x2="9.9" y2="9.9" />
        <line x1="14.1" y1="14.1" x2="18.1" y2="18.1" />
        <line x1="5.9" y1="18.1" x2="9.9" y2="14.1" />
        <line x1="14.1" y1="9.9" x2="18.1" y2="5.9" />
      </svg>
    </div>
  );
}
