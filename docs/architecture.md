> **Historical, kept on purpose.** Written in April 2026 when the product was
> called Helm. Much of the detail below is now wrong: the module list, the table
> names, and roughly eighteen references to a schema that was removed in
> September. It is kept because the reasoning is still the reasoning, and
> because the three-layer model and the "modular suite with a shared spine"
> framing are what the product still is.
>
> Do not treat anything below as a description of the current system. For that,
> read the migrations and the comments in `lib/spine/`.
